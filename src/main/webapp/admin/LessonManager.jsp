            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <font color="white">
                            ...
                            <a class="btn btn-primary btn-sm" href="<%=basePath%>ManagerServlet?method=findAllLessonData"><i class="fas fa-folder"></i>
                               	添加	
                                </a>
                            </font>
                ...
                <div class="card-body p-0">
                    <table class="table table-striped projects">
                        ...
                        <tr align="center" >
                        <td><input class="chk" name="chk" type="checkbox" value="${lesson.lesson_id }"/></td>
                            <td>${lesson.lesson_id }</td>
                            <td><a>${lesson.lesson_name }</a></td>
                            <td><a>${lesson.teacher_name }</a></td>
                            <td>${lesson.students_number }</td>
                            <td>${lesson.videos_number }</td>
                            <td>
			       				<img src="<%=picPath%>${lesson.lesson_photo }" width="40px" height="40px"/>
			       			</td>
                            <td>${lesson.description }</td>
                            
                             <td><fmt:formatDate value="${lesson.lesson_create_time }" pattern="yyyy-MM-dd" /></td>
                            <td>
                            <c:if test="${lesson.flag == 0}">未结课</c:if>
                            <font color="red"><c:if test="${lesson.flag == 1}">已结课</c:if></font>
			        			
							</td>
							
                            <td class="project-actions">
                                <a class="btn btn-info btn-sm" href="<%=basePath %>ManagerServlet?method=updateLessonToShow&lesson_id=${lesson.lesson_id }">
...
<script type="text/javascript">
	...
	function batchDelete() {
		...
		if(confirm("您是否确定要删除所选用户信息？")){
			window.location.href="<%=basePath%>ManagerServlet?method=deleteManyLessons&uids="+uids;
		}
	}
</script>
<!-- Bootstrap 4 -->
