                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <c:forEach items="${test_totalList }" var="test">
                                        <%--                                        <fmt:parseDate value="${test.begin_time}" pattern="yyyy-MM-dd HH:mm:ss"--%>
                                        <%--                                                       var="testBeginTime"/>--%>
                                        <%--                                        <fmt:parseDate value="${test.end_time}" pattern="yyyy-MM-dd HH:mm:ss"--%>
                                        <%--                                                       var="testEndTime"/>--%>
                                        <tr>
                                            ...
                                            <td>${test.end_time}</td>
                                            <td>
                                                <c:if test="${test.begin_time.time-nowDate>0}">
                                                    <button type="button" class="btn btn-block btn-secondary disabled">
                                                        考试未开始
                                                    </button>
                                                </c:if>
                                                <c:if test="${test.end_time.time -nowDate<0}">
                                                    <button type="button" class="btn btn-block btn-secondary disabled">
                                                        考试已结束
                                                    </button>
                                                </c:if>
                                                <c:if test="${test.end_time.time -nowDate>0 && test.begin_time.time-nowDate<0}">
                                                    <button type="button" class="btn btn-block btn-success" onclick="location.href='StudentServlet?method=showLearningExamDetail&testID=${test.test_id}'">进入考试
                                                    </button>
                                                </c:if>
                                            </td>
                                        </tr>
...
<script>
    ...
    function enterExam() {
        var flag= confirm("确认开始考试");
        if(flag){
            var testID= $(this).prop("id");
            // location.href="StudentServlet?method=showLearningExamDetail&testID=";
            alert("OK");
        }
    }
